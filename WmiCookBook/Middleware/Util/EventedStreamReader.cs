...
namespace WmiCookBook.Middleware.Util
{
    /// <summary>
    ...
    internal class EventedStreamReader
    {
        ...
        private async Task Run()
        {
            ...
            while (true)
            {
                ...
                var chunkStr = _linesBuffer.Append(buf);
                foreach (var line in chunkStr.ToString().Split('\n'))
                {
                    if (!string.IsNullOrWhiteSpace(line)) OnCompleteLine(line);
                }
                _linesBuffer.Clear();
            }
        }
